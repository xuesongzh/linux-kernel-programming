...
typedef volatile struct {
    struct {
        uint32_t Load;     /* +0x00 load register */
        uint32_t Value;    /* +0x04 current time register */
        uint32_t Control;  /* +0x08 control register */
        uint32_t IntClr;   /* +0x0C interrupt clear register */
        uint32_t RIS;      /* +0x10 raw interrupt register */
        uint32_t MIS;      /* +0x14 masked interrupt status register */
        uint32_t BGLoad;   /* +0x18 background load register */

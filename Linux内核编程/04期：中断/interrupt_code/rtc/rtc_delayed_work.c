...
static void rtc_delayed_work(struct work_struct *work) 
{
    struct rtc_time tm;
...
static irqreturn_t  rtc_alarm_handler(int irq, void *dev_id)
{
    ...
    set_rtc_alarm(regs);
    printk("%s\n", __func__);
    schedule_delayed_work(&rtc_work, 3*HZ);
    return IRQ_HANDLED;
}
...
static int __init rtc_init(void)
{
    ...
    INIT_DELAYED_WORK(&rtc_work, rtc_delayed_work);

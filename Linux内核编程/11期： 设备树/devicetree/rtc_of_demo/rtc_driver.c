...
static int rtc_driver_probe(struct platform_device *pdev)
{
    ...
    printk("%s: probe and init device: %s\n", __func__, pdev->name);
    
    res_mem = platform_get_resource(pdev, IORESOURCE_MEM, 0);
    if (res_mem)
        printk("get resource succuss!\n");
    else {
    ...
    device_irq = res_irq->start;
    //device_irq = platform_get_irq(pdev, 0);
...
static int rtc_driver_remove(struct platform_device *pdev)
{
...
static const struct of_device_id of_match_rtc[] = {
    { .compatible = "arm-pl031", }, /* don't use "arm,pl031"  not work*/
    {
    }
};
#endif
...
MODULE_DESCRIPTION("create and register rtc driver use DT interface");
